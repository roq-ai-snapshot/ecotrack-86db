  ...
  const { data, error, isLoading, mutate } = useSWR<BusinessOrganizationInterface[]>(
    ...
    () =>
      getBusinessOrganizations({
        relations: [
          'user',
          'carbon_footprint.count',
          'employee_feedback.count',
          'sustainability_goal.count',
          'sustainability_initiative.count',
          'waste_management.count',
        ],
      }),
  );
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr key={record.id}>
                    ...
                    {hasAccess('carbon_footprint', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.carbon_footprint}</Td>
                    )}
                    {hasAccess('employee_feedback', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.employee_feedback}</Td>
                    )}
                    {hasAccess('sustainability_goal', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.sustainability_goal}</Td>
                    )}
                    {hasAccess('sustainability_initiative', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.sustainability_initiative}</Td>
                    )}
                    {hasAccess('waste_management', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.waste_management}</Td>
                    )}
                    {hasAccess('business_organization', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                      <Td>
